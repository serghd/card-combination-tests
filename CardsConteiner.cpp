...
CardsConteiner::CardsConteiner(uint8_t cards_cells_count, QWidget* parent):
 QWidget(parent)
{
...
int8_t CardsConteiner::findCard(const QRect &pieceRect) const
{
   for (int8_t i = 0; i < _card_rects.size(); ++i)
   {
...
void CardsConteiner::addCard(QPixmap pixmap, int8_t card)
{
   int offset(0);
   for (int i = 0; i < _card_rects.size(); ++i) {
    offset += CARD_WIDTH;
