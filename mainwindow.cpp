...
{
 int8_t table_cards_count = _table_cards_wt->_cards.size();
 int8_t player_cards_count = _player_cards_wt->_cards.size();
 e_game_type game_type =
   (_ui->bgGameTypes->checkedId() == 1) ? e_game_type::texas_holdem : e_game_type::omaha;

 if (table_cards_count != 5)
 {
  QMessageBox::warning(this,
                       trUtf8("Warning"),
                       trUtf8("Wrong table cards number"),
 ...
 if (game_type == e_game_type::texas_holdem)
 {
  if (player_cards_count != 2)
  {
   QMessageBox::warning(this,
                       trUtf8("Warning"),
                       trUtf8("Wrong cards number of Holdem-player"),
					   QMessageBox::Ok, QMessageBox::Ok);
   return;
  }
 }
 ...
 else
 {
  if (player_cards_count != 4)
  {
   QMessageBox::warning(this,
                       trUtf8("Warning"),
                       trUtf8("Wrong cards number of Omaha-player"),
					   QMessageBox::Ok, QMessageBox::Ok);
   return;
  }
 }
...
void MainWindow::logCards()
{
 if ( (_table_cards.size() == 0) ||
  ...
  QMessageBox::information(this,
                           trUtf8("Warning"),
                           trUtf8("Not enought data for writing into log"),
						   QMessageBox::Ok, QMessageBox::Ok);
  return;
 }
 ...
 if (!file.open(QIODevice::Append | QIODevice::Text))
 {
  QMessageBox::critical(this,
                        trUtf8("Error"),
                        trUtf8("Error of log-file opening"),
						QMessageBox::Ok, QMessageBox::Ok);
  return;
 }
 ...
 for (const auto& c_card: _table_cards)
 {
  int card = (int)c_card;
  table_cards_unicode += cards_info::get_card(card) + " ";
 ...
 for (const auto& c_card: _hand_cards)
 {
  int card = (int)c_card;
  hand_cards_unicode += cards_info::get_card(card) + " ";
 ...
 for (const auto& c_card: _win_combination._cards)
 {
  int card = (int)c_card;
  win_cards_unicode += cards_info::get_card(card) + " ";
 ...
 // game type
 QString game_type = cards_info::get_str_game_type(_game_type);

 // combination name
 QString comb_name = cards_info::get_str_win_combination(_win_combination._combination);

 out << QDateTime::currentDateTime().toString("[yyyy.MM.dd, hh:mm:ss] ")
	 << endl
	 << game_type << ", " << trUtf8("выигрышная комбинация - ") << comb_name
	 << trUtf8(", карты: ") << win_cards_unicode << " (" << win_cards_orig << "); " << endl
	 << trUtf8("карты стола: ") << table_cards_unicode << " (" << table_cards_orig << "); " << endl
	 << trUtf8("карты игрока: ") << hand_cards_unicode << " (" << hand_cards_orig << ") " << endl
	 << "-------------------------------------------------------------------------------"
	 << endl << endl;
 ...
 QMessageBox::information(this,
                          trUtf8("error log"),
                          trUtf8("Record saved successfully"),
						  QMessageBox::Ok, QMessageBox::Ok);
}
...
{
 QString combname = cards_info::get_str_win_combination(comb);
 QString str_combination = trUtf8("Win combination: <b>") + combname + "</b>";
