...
namespace ns_cards_info
{
   ...
   inline QString get_card(int8_t card)
   {
      QString suit;
      QString rating;
   ...
   namespace ns_table
   {
      ...
      struct cards_combination_t
      {
         cards_combination_t(e_cards_combination comb, cards c):
          // default combination, don't delete!
         _combination(comb),
         _cards(c) { }
         ...
         {
            if (_combination < cc._combination) { return -1; }
            if (_combination > cc._combination) { return 1; }
...
inline ns_api::ns_table::cards_combination_t best_combination(const ns_api::ns_table::cards& cards)
{
   ...
   for (size_t idx = 0; idx < cards.size(); ++idx)
   {
      ...
      int count = static_cast<int>(std::count_if(cards.begin(), cards.end(), [&](ns_api::ns_table::card card){ return card % 13 == currentRating; }));
   ...
   {
      ns_api::ns_table::cards& sub = table[static_cast<int>(count)];
      ...
      switch(count)
      {
        ...
        default: // something going wrong, should be another value
           throw std::runtime_error("Wrong poker-eval algorithm");
      }
   }
